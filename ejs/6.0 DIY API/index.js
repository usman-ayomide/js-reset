...
app.patch("/jokes/:id", (req, res) => {
  ...
  const oldJoke = jokes[jokeId];

  const newJoke = {
    "text": req.body.text || oldJoke.text,
    "type": req.body.type || oldJoke.type,
    "id": parseInt(id)
  }
  jokes[jokeId] = newJoke;
...
app.delete("/jokes/all", (req, res) => {
  ...
  if(key) {
   ...
   res.status(404).json({error: "You cant do that"}); 
  }
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if(jokeId !== -1){
    ...
    res.status(404).json({error: "no joke with that id"});
  }
});
